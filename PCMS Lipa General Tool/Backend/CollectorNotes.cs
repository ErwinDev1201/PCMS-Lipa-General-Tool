...
namespace PCMS_Lipa_General_Tool.Class
{
	public class CollectorNotes
	{
        ...
        public bool TryGetNotesCountsAndAverage(
    string tableName,
    out int todayCount,
    out int monthCount,
    out double averagePerDay,
    out string errorMessage)
        ...
        SELECT 
            ...
            COUNT(CASE 
                      ...
                      THEN 1 
                  END) AS MonthCount
        FROM [{tableName.Replace("]", "]]")}] -- safe-escaped
        ...
        WHERE [Date] IS NOT NULL
          ...
          AND MONTH([Date]) = MONTH(GETDATE());";
            ...
            try
            {
                ...
                averagePerDay = loginDaysCount > 0
                    ? Math.Round((double)monthCount / loginDaysCount, 2)
                    : 0;
            ...
            {
                notif.LogError("TryGetNotesCountsAndAverage_SQL", "N/A", "CollectorNotes", "N/A", sqlEx);
                errorMessage = "Database error occurred while fetching note counts.";
            }
            catch (Exception ex)
            {
                notif.LogError("TryGetNotesCountsAndAverage", "N/A", "CollectorNotes", "N/A", ex);
                errorMessage = "Unexpected error occurred while processing note counts.";
            }
...
{whereClause}";
            ...
            return (false, dtResult, string.Empty); // Return empty result on error
        }
            ...
            string provider)
        {
           ...
           // string errorMessage = string.Empty;
            ...
            return (false, data, string.Empty);
        }
        ...
        public static bool DoesTableExist(string tableName)
        {
            try
            {
                string connStr = db.GetAgingDbConnection();
                using var con = new SqlConnection(connStr);
                using var cmd = new SqlCommand("SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = @Table", con);
            ...
            {
                notif.LogError($"DoesTableExist - SQL)", "N/A", "CollectorNotes", "N/A", ex);
                return false;
            }
            catch (Exception ex)
            {
                notif.LogError($"DoesTableExist - System)", "N/A", "CollectorNotes", "N/A", ex);
                return false;
            }
        }

        public (bool isSuccess, DataTable result, string errorMessage) FetchNotesData(
            string tableName,
            string keyword,
            string modeOption, // "Trans" or "Aging"
            string dateMode,   // "Today" or "Month"
            string exNotes,    // only for Aging
            DateTime? startDate,
            DateTime? endDate,
            string pageTab,
            string insurance,
            ...
            string provider)
        {
            ...
            if (string.IsNullOrWhiteSpace(tableName))
                return (false, dtResult, "Employee name (table) is missing. Please select a valid provider.");
            ...
            WHERE TABLE_SCHEMA = 'dbo' AND TABLE_NAME = @TableName", con))
                {
                ...
                bool isTrans = modeOption == "Trans";
                bool isAging = modeOption == "Aging";

                //added cache

                bool hasKeyword = !string.IsNullOrWhiteSpace(keyword);
                bool hasInsurance = !string.IsNullOrWhiteSpace(insurance) && !insurance.Equals("All", StringComparison.OrdinalIgnoreCase);
                bool hasProvider = !string.IsNullOrWhiteSpace(provider) && !provider.Equals("All", StringComparison.OrdinalIgnoreCase);
                bool hasPatientType = !string.IsNullOrWhiteSpace(patientType) && !patientType.Equals("All", StringComparison.OrdinalIgnoreCase);
                bool excludeNotes = isAging && exNotes?.Equals("excludeNotes", StringComparison.OrdinalIgnoreCase) == true;
                ...
                bool useToday = false, useMonth = false;
                bool skipDateFilter = string.Equals(pageTab, "AllNotes", StringComparison.OrdinalIgnoreCase) &&
                                      !startDate.HasValue && !endDate.HasValue;
                ...
                {
                    if (startDate.HasValue)
                        whereConditions.Add(isTrans ? "[Date] >= @StartDate" : "[Time Stamp] >= @StartDate");

                    if (endDate.HasValue)
                        whereConditions.Add(isTrans ? "[Date] <= @EndDate" : "[Time Stamp] <= @EndDate");

                    if (!startDate.HasValue && !endDate.HasValue)
                    {
                        useMonth = dateMode?.Equals("Month", StringComparison.OrdinalIgnoreCase) == true;
                        useToday = dateMode?.Equals("Today", StringComparison.OrdinalIgnoreCase) == true && isTrans;
                ...
                if (!skipDateFilter)
                {
                    ...
                    if (!startDate.HasValue && !endDate.HasValue && useMonth)
                    {
                        DateTime firstOfMonth = new(DateTime.Now.Year, DateTime.Now.Month, 1);
                        DateTime lastOfMonth = firstOfMonth.AddMonths(1).AddDays(-1).Date.AddDays(1).AddSeconds(-1);
                        cmd.Parameters.AddWithValue("@MonthStart", firstOfMonth);
            ...
            {
                notif.LogError($"FetchNotesData - SQL ({modeOption})", "N/A", "CollectorNotes", "N/A", ex);
            }
            catch (Exception ex)
            {
                notif.LogError($"FetchNotesData - System ({modeOption})", "N/A", "CollectorNotes", "N/A", ex);
            }
     ...
     out string errorMessage)
        {
            ...
            {
                whereConditions.Add("[Insurance] LIKE @Insurance");
                parameters["@Insurance"] = $"%{insuranceName}%";
            }

            if (!string.IsNullOrWhiteSpace(patientType) && !patientType.Equals("All", StringComparison.OrdinalIgnoreCase))
            {
                whereConditions.Add("[Type] = @PatientType");
                parameters["@PatientType"] = patientType;
            }
            ...
            string query = modeOption?.Equals("Aging", StringComparison.OrdinalIgnoreCase) == true
            ...
            try
            {
                ...
                lblCount = $"Total records: {result.Rows.Count:N0}";
                return true;
            }
            catch (SqlException sqlEx)
            {
                errorMessage = "A database error occurred while fetching notes.";
                notif.LogError("TryGetAllNotes_SQL", empName, "BillReview", "N/A", sqlEx);
            }
            catch (Exception ex)
            {
                errorMessage = "An unexpected error occurred while fetching notes.";
                notif.LogError("TryGetAllNotes_System", empName, "BillReview", "N/A", ex);
            }
