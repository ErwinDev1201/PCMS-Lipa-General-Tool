	...
	public class Adjuster
	{
		...
		public void AdjusterDBRequest(string request, RadTextBox adjID, RadTextBox insuranceName, RadTextBox adjusterName, RadTextBox phoneNo, RadTextBox ext, RadTextBox faxNo, RadTextBox email, RadTextBox supervisorName, RadTextBoxControl remarks, string empName)
		{
			...
			try
			{
				...
				// Log activity
				logs = $"{empName} {request.ToLower()}d Adjuster ID: {adjID.Text}";
				message = $"Done! {adjID.Text} has been successfully {request.ToLower()}d.";
				task.AddActivityLog(message, empName, logs, $"{request.ToUpper()} ADJUSTER INFORMATION");
				task.SendToastNotifDesktop(message);
			}
			catch (Exception ex)
			{
				task.LogError($"adjusterDBRequest {request}", empName, "Adjuster", adjID.Text, ex);
