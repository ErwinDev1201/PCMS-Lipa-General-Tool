...
namespace PCMS_Lipa_General_Tool__WinForm_
{
	...
	public partial class frmCollectors : RadForm
	{
		...
		public frmCollectors(string tablename, string empName, string userName, string userAccess, string empID, string officeLoc, string empStat, string position)
		{
			InitializeComponent();
            //SetTheme(themeName);	
	...
	EmpName);
		...
		private void LoadNotesCountsAndAverage()
		{
            ...
            {
                lblCountNotes.Text = "Today's Notes: 0";
                lblMonthly.Text = "Month's Notes: 0";
                lblAverage.Text = "Average per Day: 0.00";
                return;
            }

            int todayNotes, monthNotes;
            double avgNotesPerDay;
            string errorMessage;

            bool isSuccess = notes.TryGetNotesCountsAndAverage(
                tableName,
                out todayNotes,
                out monthNotes,
                out avgNotesPerDay,
            ...
            if (!isSuccess)
            {
                //RadMessageBox.Show(errorMessage, "Notes Summary", MessageBoxButtons.OK, RadMessageIcon.Info);
                lblCountNotes.Text = "Today's Notes: 0";
                lblMonthly.Text = "Month's Notes: 0";
                lblAverage.Text = "Average per Day: 0.00";
		...
		private void dgCurrentNotes_MouseDoubleClick(object sender, MouseEventArgs e)
		{
			...
			else
			{
				...
				try
				{
                    ...
                    modAdj.txtPrevNotes.Enabled = string.Equals(Position, "Collector", StringComparison.OrdinalIgnoreCase);
					//modAdj.cmbProviderList.Text = selectedRow.Cells["Provider"].Value?.ToString() ?? string.Empty;
					modAdj.txtPatientType.Enabled = string.Equals(Position, "Collector", StringComparison.OrdinalIgnoreCase);
                    modAdj.txtChartNo.Text = selectedRow.Cells["Chart"].Value?.ToString() ?? string.Empty;
                    modAdj.txtChartNo.Enabled = string.Equals(Position, "Collector", StringComparison.OrdinalIgnoreCase);
                    modAdj.cmbProviderList.Text = selectedRow.Cells["Provider"].Value?.ToString() ?? string.Empty;
                    modAdj.cmbProviderList.Enabled = string.Equals(Position, "Collector", StringComparison.OrdinalIgnoreCase);
                    if (Position != "Collector")
					{
    ...
    //    public bool TryCreateCollectors(
    //string empName, string userName, string accessLevel, string employeeID,
    //string officeLoc, string employeeStat, string position,
    //string tableName,
    //out frmCollectors form,
    ...
    //        if (!notes.DoesTableExistD(tableName)) // Assume you have this utility
    //        {
    //            errorMessage = $"The data for '{tableName}' could not be found.";
    //            return false;
    //        }
	//
    //        form = new frmCollectors(empName, userName, accessLevel, employeeID,
    //                                 officeLoc, employeeStat, position, tableName);
    //        return true;
    //    }
    ...
    DateTime.DaysInMonth(DateTime.Now.Year, DateTime.Now.Month));
        ...
        private void LoadAllNotesPerProvider()
        {
            ...
            try
            {
                bool isSuccess = notes.TryGetAllNotes(
                    tableName, modeOption, EmpName,
                    patientName, insurance, keyword,
                    patientType, provider,
                    out notesTable, out resultCount, out errorMsg);
                ...
                if (!isSuccess)
                {
					...
					{
                        RadMessageBox.Show(errorMsg, "Notes Loader", MessageBoxButtons.OK, RadMessageIcon.Info);
                    }
                    return;
                }
        ...
        private void cmbEmployeeName_SelectedIndexChanged(object sender, Telerik.WinControls.UI.Data.PositionChangedEventArgs e)
        {
			if (pgViewCollectorNotes.SelectedPage.Text == "All Notes")
			{
				if (chkDateFilter.Checked)
                {
                    GetAllNotesinAllProvider();
